 ...
 * una pagina de login server-side. La contraseña viaja en un header
 * (x-zs-pass) via fetch — no en el body, porque el Edge Middleware no
 * lee el body de forma confiable. Password correcto -> Set-Cookie + el
 * cliente recarga.
 */
export const config = {
  matcher: ['/', '/index.html'],
...
<form class="box" id="f">
  ...
  <input type="password" id="p" placeholder="Contraseña" autofocus autocomplete="off">
...
f.addEventListener('submit',function(ev){
  ...
  fetch('/',{method:'POST',headers:{'x-zs-pass':p.value}}).then(function(r){
    if(r.ok){
      document.cookie='zs_ok=1; Path=/; SameSite=Lax; Secure; Max-Age=604800';
      document.cookie='zs_fresh=1; Path=/; SameSite=Lax; Secure; Max-Age=20';
      location.replace('/');
    }
    else{b.disabled=false;e.textContent='Contraseña incorrecta.';p.value='';p.focus();}
  }).catch(function(){b.disabled=false;e.textContent='Error de red. Reintentá.';});
});
</script>
</body></html>`;
}
...
export default function middleware(request) {
  ...
  if (request.method === 'POST') {
    const pass = request.headers.get('x-zs-pass') || '';
    if (pass === PASS) {
      // Solo la cookie de gate (HttpOnly). Las cookies legibles por JS
      // (zs_ok / zs_fresh) las setea la pagina de login antes de recargar,
      // para no depender de multiples Set-Cookie en una sola respuesta.
      return new Response('ok', {
        status: 200,
        headers: {
          'set-cookie': `zs_auth=${TOKEN}; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=${MAXAGE}`,
